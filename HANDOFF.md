...
| Area | Notes |
|---|---|
| Frame, status bar, lock screen, Control Center | From the earlier iFruit rework commit. Lock screen swipe fixed (quick flicks now unlock). |
...
| **Weather** (`WEATHER`) | TS client `cl_weather.ts` | Reads GTA weather/rain/wind; lb-phone's temperature table on a smooth day curve; outlook from Renewed-Weathersync `GlobalState.weather.time`. |
